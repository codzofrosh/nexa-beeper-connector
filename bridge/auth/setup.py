...
INSTAGRAM_BRIDGE_CONFIG     = _repo_root / "bridge" / "instagram" / "config.yaml"
INSTAGRAM_REGISTRATION_FILE = _repo_root / "bridge" / "instagram" / "instagram-registration.yaml"
INSTAGRAM_PORT              = 29320

TWITTER_BRIDGE_CONFIG     = _repo_root / "bridge" / "twitter" / "config.yaml"
TWITTER_REGISTRATION_FILE = _repo_root / "bridge" / "twitter" / "twitter-registration.yaml"
TWITTER_PORT              = 29321
...
def _patch_linkedin_config(config_file: Path) -> None:
    ...
    patches = {
        ...
        r"(^\s*hostname:\s*)127\.0\.0\.1": r"\g<1>0.0.0.0",
        # Port — match whatever default the bridge image uses
        r"(^\s*port:\s*)\d+\b": rf"\g<1>{LINKEDIN_PORT}",
...
# ─── Step 1d: Instagram (mautrix-meta) ───────────────────────────────────────

def step_generate_instagram_registration() -> None:
    """
    Generate the mautrix-meta appservice registration for Instagram mode.

    Phase A  (Docker -e)  Pull example config.yaml from the bridge image.
    Phase B  (Python)     Write instagram-registration.yaml directly.
    """
    _print("\n── Step 1d: Generate mautrix-meta (Instagram) appservice registration ──")

    bridge_dir   = _repo_root / "bridge" / "instagram"
    bridge_dir.mkdir(parents=True, exist_ok=True)
    docker_mount = _to_docker_path(bridge_dir)
    config_file  = bridge_dir / "config.yaml"

    if INSTAGRAM_REGISTRATION_FILE.exists():
        _info("instagram-registration.yaml already exists — skipping generation")
        _inject_tokens_into_instagram_config()
        return

    if config_file.exists():
        backup = bridge_dir / "config.yaml.bak"
        config_file.rename(backup)
        _info("Backed up existing instagram config.yaml → config.yaml.bak")

    _info("Phase A: generating example config.yaml from mautrix-meta bridge image (-e)…")
    result = _run([
        "docker", "run", "--rm",
    ...
    if not config_file.exists():
        ...
        if backup.exists():
            backup.rename(config_file)
            _warn(
                "Docker volume mount failed — restored backup instagram config.yaml.\n"
                "  Ensure Docker Desktop has file sharing enabled for this drive."
            )
        else:
            _fail(
                "Instagram bridge config was not created and no backup exists.\n"
                f"Docker stdout: {result.stdout}\nstderr: {result.stderr}"
            )
    else:
        _ok("Generated mautrix-meta config.yaml")

    _patch_instagram_config(config_file)

    _info("Phase B: writing instagram-registration.yaml…")
    _write_instagram_registration_yaml()
    _inject_tokens_into_instagram_config()


def _write_instagram_registration_yaml() -> None:
    """Create instagram-registration.yaml with freshly-generated tokens."""
    as_token = secrets.token_hex(32)
    hs_token = secrets.token_hex(32)
    ...
    content = (
        "# mautrix-meta (Instagram) appservice registration\n"
        "# Generated by bridge/auth/setup.py — do not edit manually.\n"
        "# Tokens here must match as_token / hs_token in config.yaml.\n"
        f"id: meta\n"
        f"url: http://mautrix-instagram:{INSTAGRAM_PORT}\n"
        f'as_token: "{as_token}"\n'
        f'hs_token: "{hs_token}"\n'
        f"sender_localpart: instagrambot\n"
        f"rate_limited: false\n"
        f"namespaces:\n"
        f"  users:\n"
        f"    - regex: '@instagram_.+:{sn}'\n"
        f"      exclusive: true\n"
        f"  aliases: []\n"
        f"  rooms: []\n"
        f"de.sorunome.msc2409.push_ephemeral: true\n"
    ...
    INSTAGRAM_REGISTRATION_FILE.write_text(content, encoding="utf-8")
    _ok(f"Generated {INSTAGRAM_REGISTRATION_FILE.name}")


def _patch_instagram_config(config_file: Path) -> None:
    """Overwrite key fields in the mautrix-meta bridge-generated config.yaml for Instagram."""
    text = config_file.read_text(encoding="utf-8")

    patches = {
        # Set mode to instagram (field is indented inside network: block)
        r"(^\s+mode:\s*)$":                               r"\g<1>instagram",
        r"(^\s+mode:\s*)\S+":                             r"\g<1>instagram",
        # Homeserver — point to Synapse on the internal Docker network
        r"(^\s*address:\s*)https?://example(?:\.com|\.localhost(?::\d+)?)": r"\g<1>http://conduit:8008",
        r"(^\s*domain:\s*)example(?:\.com|\.localhost)":  rf"\g<1>{SERVER_NAME}",
        # Appservice — bridge's own address on the Docker network
        r"(^\s*address:\s*)http://localhost:\d+":         rf"\g<1>http://mautrix-instagram:{INSTAGRAM_PORT}",
        # Bind on all interfaces
        r"(^\s*hostname:\s*)127\.0\.0\.1":                r"\g<1>0.0.0.0",
        # Port — match whatever default the bridge image uses
        r"(^\s*port:\s*)\d+\b":                           rf"\g<1>{INSTAGRAM_PORT}",
        # Database — use SQLite in the bridge data volume
        r"(^\s*type:\s*)postgres":                        r"\g<1>sqlite3-fk-wal",
        r"(^\s*uri:\s*)postgres://[^\n]+":                r"\g<1>file:/data/mautrix-meta.db?_txlock=immediate",
        # Permissions
        r'(\s*"\*"\s*:\s*)relay':                         r'\g<1>user',
        r'"@admin:example\.com"':                         f'"@{ADMIN_USERNAME}:{SERVER_NAME}"',
        r'"example\.com"':                                f'"{SERVER_NAME}"',
        # Bot username
        r"(^\s*username:\s*)metabot":                     r"\g<1>instagrambot",
        r"(sender_localpart:\s*)metabot":                 r"\g<1>instagrambot",
        # Username template
        r"(username_template:\s*)meta_":                  r"\g<1>instagram_",
    }
    for pattern, replacement in patches.items():
        text, n = re.subn(pattern, replacement, text, flags=re.MULTILINE)
        if n:
            _info(f"  patched: {pattern[:60]}")

    config_file.write_text(text, encoding="utf-8")
    _ok("Patched instagram config.yaml with homeserver / domain / Instagram mode / permissions")


def _inject_tokens_into_instagram_config() -> None:
    """Copy as_token and hs_token from instagram-registration.yaml into config.yaml."""
    as_token = _read_yaml_value(INSTAGRAM_REGISTRATION_FILE, "as_token")
    hs_token = _read_yaml_value(INSTAGRAM_REGISTRATION_FILE, "hs_token")

    if not as_token or not hs_token:
        _warn("Could not read instagram tokens — config.yaml may need manual update")
        return

    _set_yaml_value(INSTAGRAM_BRIDGE_CONFIG, "as_token", as_token)
    _set_yaml_value(INSTAGRAM_BRIDGE_CONFIG, "hs_token", hs_token)
    _set_yaml_value(INSTAGRAM_BRIDGE_CONFIG, "domain", SERVER_NAME)

    text = INSTAGRAM_BRIDGE_CONFIG.read_text(encoding="utf-8")
    text = text.replace(
        '"@admin:localhost": "admin"',
        f'"@{ADMIN_USERNAME}:{SERVER_NAME}": "admin"',
    )
    INSTAGRAM_BRIDGE_CONFIG.write_text(text, encoding="utf-8")

    _ok("Tokens injected into bridge/instagram/config.yaml")


def step_start_instagram_bridge() -> None:
    _print("\n── Step 4c: Start mautrix-meta (Instagram) bridge ──")

    result = _run(
        ["docker-compose", "up", "-d", "mautrix-instagram"],
        cwd=str(_repo_root),
    )
    ...
    if result.returncode != 0:
        _fail(f"Could not start Instagram bridge:\n{result.stderr}")

    _ok("mautrix-instagram bridge container started")
    _info("Giving the bridge 5 seconds to connect to Synapse…")
    time.sleep(5)

    r = _run(["docker", "ps", "--filter", "name=nexa-instagram-bridge", "--format", "{{.Status}}"])
    if "Up" in r.stdout:
        _ok("Instagram bridge container is running")
    else:
        _warn(
            "Instagram bridge container may have exited.\n"
            "  Check: docker logs nexa-instagram-bridge\n"
            "  Common cause: as_token / hs_token mismatch between config.yaml and registration.yaml"
        )


# ─── Step 1e: X / Twitter (mautrix-twitter) ──────────────────────────────────

def step_generate_twitter_registration() -> None:
    """
    Generate the mautrix-twitter appservice registration.

    Phase A  (Docker -e)  Pull example config.yaml from the bridge image.
    Phase B  (Python)     Write twitter-registration.yaml directly.
    """
    _print("\n── Step 1e: Generate mautrix-twitter (X) appservice registration ──")

    bridge_dir   = _repo_root / "bridge" / "twitter"
    bridge_dir.mkdir(parents=True, exist_ok=True)
    docker_mount = _to_docker_path(bridge_dir)
    config_file  = bridge_dir / "config.yaml"

    if TWITTER_REGISTRATION_FILE.exists():
        _info("twitter-registration.yaml already exists — skipping generation")
        _inject_tokens_into_twitter_config()
        return

    if config_file.exists():
        backup = bridge_dir / "config.yaml.bak"
        config_file.rename(backup)
        _info("Backed up existing twitter config.yaml → config.yaml.bak")

    _info("Phase A: generating example config.yaml from mautrix-twitter bridge image (-e)…")
    result = _run([
        "docker", "run", "--rm",
        "--entrypoint", "/usr/bin/mautrix-twitter",
        "-v", f"{docker_mount}:/data",
        "dock.mau.dev/mautrix/twitter:latest",
        "-c", "/data/config.yaml",
        "-e",
    ])
    ...
    if not config_file.exists():
        ...
        if backup.exists():
            backup.rename(config_file)
            _warn(
                "Docker volume mount failed — restored backup twitter config.yaml.\n"
                "  Ensure Docker Desktop has file sharing enabled for this drive."
            )
        else:
            _fail(
                "Twitter bridge config was not created and no backup exists.\n"
                f"Docker stdout: {result.stdout}\nstderr: {result.stderr}"
            )
    else:
        _ok("Generated mautrix-twitter config.yaml")

    _patch_twitter_config(config_file)

    _info("Phase B: writing twitter-registration.yaml…")
    _write_twitter_registration_yaml()
    _inject_tokens_into_twitter_config()


def _write_twitter_registration_yaml() -> None:
    """Create twitter-registration.yaml with freshly-generated tokens."""
    as_token = secrets.token_hex(32)
    hs_token = secrets.token_hex(32)
    ...
    content = (
        "# mautrix-twitter appservice registration\n"
        "# Generated by bridge/auth/setup.py — do not edit manually.\n"
        "# Tokens here must match as_token / hs_token in config.yaml.\n"
        f"id: twitter\n"
        f"url: http://mautrix-twitter:{TWITTER_PORT}\n"
        f'as_token: "{as_token}"\n'
        f'hs_token: "{hs_token}"\n'
        f"sender_localpart: twitterbot\n"
        f"rate_limited: false\n"
        f"namespaces:\n"
        f"  users:\n"
        f"    - regex: '@twitter_.+:{sn}'\n"
    ...
    TWITTER_REGISTRATION_FILE.write_text(content, encoding="utf-8")
    _ok(f"Generated {TWITTER_REGISTRATION_FILE.name}")


def _patch_twitter_config(config_file: Path) -> None:
    """Overwrite key fields in the mautrix-twitter bridge-generated config.yaml."""
    text = config_file.read_text(encoding="utf-8")
    ...
    patches = {
        ...
        r"(^\s*domain:\s*)example(?:\.com|\.localhost)":  rf"\g<1>{SERVER_NAME}",
        # Appservice — bridge's own address on the Docker network
        r"(^\s*address:\s*)http://localhost:\d+":         rf"\g<1>http://mautrix-twitter:{TWITTER_PORT}",
        # Bind on all interfaces
        r"(^\s*hostname:\s*)127\.0\.0\.1":                r"\g<1>0.0.0.0",
        # Port — match whatever default the bridge image uses
        r"(^\s*port:\s*)\d+\b":                           rf"\g<1>{TWITTER_PORT}",
        # Database — use SQLite in the bridge data volume
        r"(^\s*type:\s*)postgres":                        r"\g<1>sqlite3-fk-wal",
        r"(^\s*uri:\s*)postgres://[^\n]+":                r"\g<1>file:/data/mautrix-twitter.db?_txlock=immediate",
        # Permissions
        r'(\s*"\*"\s*:\s*)relay':                         r'\g<1>user',
        r'"@admin:example\.com"':                         f'"@{ADMIN_USERNAME}:{SERVER_NAME}"',
        r'"example\.com"':                                f'"{SERVER_NAME}"',
        # Username template
        r"(username_template:\s*)twitter_\{\{":           r"\g<1>twitter_{{",
    }
    for pattern, replacement in patches.items():
        text, n = re.subn(pattern, replacement, text, flags=re.MULTILINE)
        if n:
    ...
    config_file.write_text(text, encoding="utf-8")
    _ok("Patched twitter config.yaml with homeserver / domain / permissions")


def _inject_tokens_into_twitter_config() -> None:
    """Copy as_token and hs_token from twitter-registration.yaml into config.yaml."""
    as_token = _read_yaml_value(TWITTER_REGISTRATION_FILE, "as_token")
    hs_token = _read_yaml_value(TWITTER_REGISTRATION_FILE, "hs_token")

    if not as_token or not hs_token:
        _warn("Could not read twitter tokens — config.yaml may need manual update")
        return

    _set_yaml_value(TWITTER_BRIDGE_CONFIG, "as_token", as_token)
    _set_yaml_value(TWITTER_BRIDGE_CONFIG, "hs_token", hs_token)
    _set_yaml_value(TWITTER_BRIDGE_CONFIG, "domain", SERVER_NAME)

    text = TWITTER_BRIDGE_CONFIG.read_text(encoding="utf-8")
    text = text.replace(
        '"@admin:localhost": "admin"',
        f'"@{ADMIN_USERNAME}:{SERVER_NAME}": "admin"',
    )
    TWITTER_BRIDGE_CONFIG.write_text(text, encoding="utf-8")

    _ok("Tokens injected into bridge/twitter/config.yaml")


def step_start_twitter_bridge() -> None:
    _print("\n── Step 4d: Start mautrix-twitter (X) bridge ──")

    result = _run(
        ["docker-compose", "up", "-d", "mautrix-twitter"],
        cwd=str(_repo_root),
    )
    if result.returncode != 0:
        result = _run(
    ...
    if result.returncode != 0:
        _fail(f"Could not start Twitter bridge:\n{result.stderr}")

    _ok("mautrix-twitter bridge container started")
    _info("Giving the bridge 5 seconds to connect to Synapse…")
    time.sleep(5)

    r = _run(["docker", "ps", "--filter", "name=nexa-twitter-bridge", "--format", "{{.Status}}"])
    if "Up" in r.stdout:
        _ok("Twitter bridge container is running")
    else:
        _warn(
            "Twitter bridge container may have exited.\n"
            "  Check: docker logs nexa-twitter-bridge\n"
            "  Common cause: as_token / hs_token mismatch between config.yaml and registration.yaml"
        )
...
async def run() -> None:
    ...
    step_generate_instagram_registration()
    step_generate_twitter_registration()
    step_generate_synapse_config()
    step_start_conduit()
    await step_register_admin()
    step_start_bridge()
    step_start_linkedin_bridge()
    step_start_instagram_bridge()
    step_start_twitter_bridge()
    step_start_sidecar()
    ...
    _print()
    _print("  To connect a user's Instagram:")
    _print()
    _print('    DM @instagrambot:localhost in Matrix')
    _print('    Send: login')
    _print('    → Bridge replies with instructions to provide Instagram cookies')
    _print('    → Open instagram.com in browser → DevTools → Application → Cookies')
    _print('    → Copy the "sessionid" cookie value and paste to the bridge bot')
    _print()
    _print("  To connect a user's X (Twitter):")
    _print()
    _print('    DM @twitterbot:localhost in Matrix')
    _print('    Send: login')
    _print('    → Bridge replies with instructions to provide X/Twitter cookies')
    _print('    → Open x.com in browser → DevTools → Application → Cookies')
    _print('    → Copy the "auth_token" and "ct0" cookie values and paste to the bridge bot')
    _print()
